...
def process_line(line, processed_ids, queue):
    ...
    try:
        for ent_name, ent in line['query_entity'].items():
            ent_info = query_entity(ent['id'])
            ent['claims'] = ent_info['claims']
            line['query_entity'][ent_name] = ent

        queue.put(line)
    except Exception as e:
        print(f"Error processing line with id {line['id']}: {e}")
...
if __name__ == "__main__":
    input_file_name = 'PopQA/turn1_question_el.json'
    output_file_name = 'PopQA/turn1_question_el_full.json'
